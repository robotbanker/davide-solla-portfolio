...
const imageLicenseUrl = `${siteUrl}/image-licensing`;
...
const serviceNameByPath = Object.freeze({
  "/services/fashion-photographer-london": "Fashion photography",
  "/services/model-portfolio-photographer-london": "Model portfolio photography",
  "/services/beauty-photographer-london": "Beauty photography",
  "/services/artist-portrait-photographer-london": "Artist portrait photography",
  "/services/fine-art-portrait-commissions": "Fine-art portrait commissions"
});
...
const projectImages = (album = {}) => {
  const seen = new Set();
  // Preserve the sequence selected by the gallery author. A cover can be
  // chosen from the middle of a story without becoming its first frame.
  const values = [...(album.images || []), ...(album.covers || [])];
  return values.filter((image) => {
    ...
    dimensions: dimensionsForImage(image.src)
  }));
};
...
const pictureMarkup = (image, index) => {
  ...
  const dimensions = image.dimensions
    ...
    : "";
  const imageMarkup = `<img src="${escapeHtml(src)}" alt="${alt}"${dimensions} loading="${loading}" decoding="async"${priority}>`;
  ...
  return `<picture>
            ...
            <img src="${escapeHtml(src)}" srcset="${responsiveSource(image.src, 720, originalImageExtension(image.src))} 720w, ${responsiveSource(image.src, 1200, originalImageExtension(image.src))} 1200w" sizes="(max-width: 760px) 100vw, 76vw" alt="${alt}"${dimensions} loading="${loading}" decoding="async"${priority}>
          </picture>`;
};
...
const jsonLdMarkup = (album, slug, images, credits, siteData) => {
  ...
  const servicePath = serviceNameByPath[album.servicePath] ? album.servicePath : "";
  ...
  const graph = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        url: canonical,
        name: pageMetadata(album, slug).title,
        description: pageMetadata(album, slug).description,
        inLanguage: "en-GB",
        isPartOf: { "@id": `${siteUrl}/#website` },
        primaryImageOfPage: { "@id": `${canonical}#primary-image` },
        ...(servicePath ? { about: { "@id": `${siteUrl}${servicePath}#service` } } : {})
      },
      {
        "@type": "ImageGallery",
        ...
        copyrightHolder: creatorReference,
        copyrightNotice: `© ${photographer.name}`,
        dateModified: cleanText(album.projectPage?.updatedAt || album.updatedAt, 40) || undefined,
        primaryImageOfPage: { "@id": `${canonical}#primary-image` },
        image: images.map((image, index) => ({
          ...
          ...(image.dimensions ? { width: image.dimensions.width, height: image.dimensions.height } : {})
        })),
        ...(review ? {
...
const pageMetadata = (album, slug) => {
  ...
  const servicePath = serviceNameByPath[album.servicePath] ? album.servicePath : "";
  const titleContext = [category, location].filter(Boolean).join(" in ");
  return {
    category,
    location,
    year,
    servicePath,
    serviceName: serviceNameByPath[servicePath] || "",
    canonical: `${siteUrl}/work/${slug}`,
    title: `${cleanText(album.title, 100)} — ${titleContext || "Photography"} | Davide Solla`,
...
const metadataRows = (metadata) => [
  ...
  ["Year", metadata.year],
  ...(metadata.servicePath ? [["Commissions", metadata.serviceName, metadata.servicePath]] : [])
].filter(([, value]) => value).map(([label, value, href]) => `<div><dt>${escapeHtml(label)}</dt><dd>${href ? `<a href="${escapeHtml(href)}">${escapeHtml(value)}</a>` : escapeHtml(value)}</dd></div>`).join("");
...
const renderProjectPage = (siteData, requestedSlug) => {
  ...
  const primaryImageDimensions = primaryImage.dimensions;
...
<html lang="en-GB" data-analytics="enabled">
  <head>
    ...
    ${primaryImageDimensions ? `<meta property="og:image:width" content="${primaryImageDimensions.width}">
    <meta property="og:image:height" content="${primaryImageDimensions.height}">` : ""}
  ...
  <body class="project-page">
    ...
    <main>
      <article>
        ...
        ${album.id === "kihaya-blues" ? `<section class="project-credits" aria-labelledby="field-notes-title">
          <div><p class="section-kicker">Field Notes</p><h2 id="field-notes-title">The sound of legacy.</h2></div>
          <p><a href="/field-notes/2026-08">Read the portrait story with Kihaya Blues founder Kyazi Lugangira.</a></p>
        </section>` : ""}
...
const handleProjectPageRequest = (req, res) => {
  ...
  if (/^\/work\/[a-z0-9-]+\/$/.test(requestUrl.pathname)) {
    res.setHeader("cache-control", "no-store");
    res.setHeader("location", requestUrl.pathname.replace(/\/+$/, ""));
    res.statusCode = 308;
    res.end();
    return;
  }
