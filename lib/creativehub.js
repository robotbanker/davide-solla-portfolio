...
const requestOrigin = (req) => {
  ...
  const protocol = proto || (req.headers.host?.startsWith("localhost") ? "http" : "https");
...
const stripeCheckoutSession = async (checkout, origin) => {
  ...
  body.set("metadata[creativehub_order_id]", String(checkout.orderId));
  body.set("metadata[creativehub_delivery_option_id]", String(checkout.deliveryOption.id));
  body.set("metadata[creativehub_delivery_charge_excluding_tax]", String(checkout.deliveryOption.deliveryChargeExcludingSalesTax || 0));
  body.set("metadata[creativehub_delivery_charge_sales_tax]", String(checkout.deliveryOption.deliveryChargeSalesTax || 0));
  body.set("metadata[creativehub_external_reference]", checkout.externalReference);
  body.set("payment_intent_data[metadata][creativehub_order_id]", String(checkout.orderId));
  body.set("payment_intent_data[metadata][creativehub_delivery_option_id]", String(checkout.deliveryOption.id));
  body.set("payment_intent_data[metadata][creativehub_delivery_charge_excluding_tax]", String(checkout.deliveryOption.deliveryChargeExcludingSalesTax || 0));
  body.set("payment_intent_data[metadata][creativehub_delivery_charge_sales_tax]", String(checkout.deliveryOption.deliveryChargeSalesTax || 0));
...
const confirmCreativehubFulfillment = async (metadata) => {
  const config = creativehubConfig();
  const orderId = parsePositiveInteger(metadata.creativehub_order_id);
  const deliveryOptionId = parsePositiveInteger(metadata.creativehub_delivery_option_id);
  const deliveryChargeExcludingSalesTax = Number(metadata.creativehub_delivery_charge_excluding_tax || 0);
  const deliveryChargeSalesTax = Number(metadata.creativehub_delivery_charge_sales_tax || 0);
  ...
  return creativehubRequest("/api/v1/orders/confirmed", {
    ...
    body: JSON.stringify({
      ...
      ExternalReference: metadata.creativehub_external_reference || ""
    })
  }, config);
};
...
const fulfillPaidStripeCheckout = async (session) => {
  ...
  return {
    fulfilled: true,
    orderId: session.metadata?.creativehub_order_id || ""
  };
};
