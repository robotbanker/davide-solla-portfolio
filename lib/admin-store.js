...
const clientFeedbackDataFile = "data/client-feedback.enc";
...
const requireClientSessionSecret = () => {
  const configuredSessionSecret = String(process.env.ADMIN_SESSION_SECRET || "").trim();
  const secret = configuredSessionSecret || adminDataSecret();

  if (!secret) {
    const error = new Error("A client session signing secret is not configured");
...
const normaliseClient = (client = {}, index) => {
  ...
  return {
    ...
    downloadEnabled: client.downloadEnabled === true,
    updatedAt: new Date().toISOString()
  };
};
...
const normalisedClientRecords = (siteData) => (Array.isArray(siteData?.clients) ? siteData.clients : [])
  .map((storedClient, index) => ({
    storedClient,
    client: normaliseClient(storedClient, index)
  }))
  .filter(({ client }) => client.name || client.email || client.lightroomUrl);

const emptyClientFeedbackData = () => ({
  version: 1,
  updatedAt: "",
  records: []
});

const normaliseFeedbackTimestamp = (value) => {
  const parsed = Date.parse(String(value || ""));
  return Number.isFinite(parsed) ? new Date(parsed).toISOString() : "";
};

const normaliseStoredFeedbackRecord = (record = {}) => {
  const clientId = cleanText(record.clientId, 80);
  const lightroomAssetId = cleanText(record.lightroomAssetId, 240);
  const rating = Number(record.rating);
  const comment = String(record.comment || "").trim().slice(0, 1500);

  if (!clientId || !lightroomAssetId || !Number.isInteger(rating) || rating < 0 || rating > 5) {
    return null;
  }
  ...
  const createdAt = normaliseFeedbackTimestamp(record.createdAt);
  const updatedAt = normaliseFeedbackTimestamp(record.updatedAt) || createdAt;

  return {
    clientId,
    lightroomAssetId,
    imageAlt: cleanText(record.imageAlt, 500),
    imageSrc: cleanText(record.imageSrc, 3000),
    rating,
    comment,
...
const normaliseClientFeedbackData = (value = {}) => {
  const recordsByImage = new Map();

  for (const candidate of Array.isArray(value.records) ? value.records : []) {
    const record = normaliseStoredFeedbackRecord(candidate);
    if (!record) continue;
    recordsByImage.set(`${record.clientId}\u0000${record.lightroomAssetId}`, record);
  }

  return {
    version: 1,
    updatedAt: normaliseFeedbackTimestamp(value.updatedAt),
    records: [...recordsByImage.values()].sort((left, right) => (
      String(right.updatedAt).localeCompare(String(left.updatedAt))
    ))
  };
};

const feedbackForClient = (feedbackData, clientId) => normaliseClientFeedbackData(feedbackData).records
  .filter((record) => record.clientId === clientId)
  .map(({ clientId: unusedClientId, ...record }) => ({ ...record }));

const mergeClientFeedback = (siteData, feedbackData) => ({
  ...siteData,
  clients: Array.isArray(siteData?.clients)
    ? siteData.clients.map((client, index) => {
      const clientId = slugify(String(client.id || client.email || client.name || `client-${index + 1}`));
      return {
        ...client,
        id: clientId,
        feedback: feedbackForClient(feedbackData, clientId)
      };
    })
    : []
});

const readClientFeedbackData = async (ref = "") => {
  try {
    const encrypted = await readDataFile(clientFeedbackDataFile, false, ref);
    return normaliseClientFeedbackData(decryptAdminData(encrypted));
  } catch (error) {
    if (isMissingFileError(error)) return emptyClientFeedbackData();
    throw error;
  }
};

const normaliseFeedbackMutation = (mutation, fallbackData) => {
  const nextData = normaliseClientFeedbackData(mutation?.data || fallbackData);
  const write = mutation?.write !== false;
  if (write) nextData.updatedAt = new Date().toISOString();
  return {
    data: nextData,
...
const updateClientFeedbackDataUnlocked = async (operation) => {
  const config = repoConfig();
  ...
  if (config.token) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const headSha = await readGithubBranchHead();
      const current = await readClientFeedbackData(headSha);
      const mutation = normaliseFeedbackMutation(operation(current), current);
      if (!mutation.write) return mutation.result;
      const content = Buffer.from(encryptAdminData(mutation.data));

      try {
        await writeGithubFiles([
          { path: clientFeedbackDataFile, content }
        ], "Update private client gallery feedback", { expectedHeadSha: headSha });
  ...
  const current = await readClientFeedbackData();
  const mutation = normaliseFeedbackMutation(operation(current), current);
  if (!mutation.write) return mutation.result;
  const fullPath = path.join(rootDir, clientFeedbackDataFile);
  await fs.mkdir(path.dirname(fullPath), { recursive: true });
  await fs.writeFile(fullPath, encryptAdminData(mutation.data), "utf8");
  return mutation.result;
};

const updateClientFeedbackData = (operation) => withRepositoryWriteLock(
  () => updateClientFeedbackDataUnlocked(operation)
);

const pruneClientFeedback = (clientIds) => {
  const activeClientIds = new Set(clientIds);

  return updateClientFeedbackData((feedbackData) => {
    const records = feedbackData.records.filter((record) => activeClientIds.has(record.clientId));
    const pruned = feedbackData.records.length - records.length;

    return {
      data: { ...feedbackData, records },
      result: { pruned },
      write: pruned > 0
    };
  });
...
const normaliseFeedbackSubmission = (body = {}) => {
  const lightroomAssetId = String(body.lightroomAssetId || "").trim();
  const rating = Number(body.rating);
  const comment = String(body.comment || "").trim();

  if (!lightroomAssetId || lightroomAssetId.length > 240) {
    const error = new Error("Choose a valid gallery image before saving feedback.");
    error.statusCode = 400;
    throw error;
  }

  if (!Number.isInteger(rating) || rating < 0 || rating > 5) {
    const error = new Error("Rating must be a whole number from 0 to 5.");
    error.statusCode = 400;
    throw error;
  }

  if (comment.length > 1500) {
    const error = new Error("Comments must be 1500 characters or fewer.");
    error.statusCode = 400;
    throw error;
  }
  ...
  return {
    ...
    rating,
    comment,
    deleting: rating === 0 && !comment
  };
};

const saveClientImageFeedback = (client, image, submission) => {
  const now = new Date().toISOString();

  return updateClientFeedbackData((feedbackData) => {
    const records = [...feedbackData.records];
    const index = records.findIndex((record) => (
      record.clientId === client.id
      && record.lightroomAssetId === submission.lightroomAssetId
    ));
    const existing = index >= 0 ? records[index] : null;

    if (submission.deleting) {
      if (index >= 0) records.splice(index, 1);
      return {
        data: { ...feedbackData, records },
        result: { deleted: true, feedback: null }
      };
    }

    const record = {
      clientId: client.id,
      lightroomAssetId: submission.lightroomAssetId,
      imageAlt: cleanText(image.alt, 500),
      imageSrc: cleanText(image.src, 3000),
    ...
    return {
      ...
      result: {
        deleted: false,
        feedback: feedbackForClient({ records: [record] }, client.id)[0]
      }
    };
  });
};

const clientCredentialMaterial = (client, storedClient = {}) => {
  const storedHash = cleanText(storedClient.passwordHash, 240);
  if (storedHash) return `hash:${storedHash}`;
  if (typeof storedClient.password === "string") return `legacy:${storedClient.password}`;
  const normalisedHash = cleanText(client?.passwordHash, 240);
  return normalisedHash ? `hash:${normalisedHash}` : "none";
};

const clientConfigurationFingerprint = (client, storedClient = {}) => {
  const sessionSecret = requireClientSessionSecret();
  const configuration = JSON.stringify({
    lightroomUrl: String(client?.lightroomUrl || ""),
    downloadEnabled: client?.downloadEnabled === true,
    credential: clientCredentialMaterial(client, storedClient)
  });
  return crypto.createHmac("sha256", sessionSecret).update(configuration).digest("base64url");
};

const createClientToken = (client, storedClient) => {
  const sessionSecret = requireClientSessionSecret();
  const payload = Buffer.from(JSON.stringify({
    purpose: "client-gallery-session",
    clientId: client.id,
    configuration: clientConfigurationFingerprint(client, storedClient),
...
const verifyClientToken = (token) => {
  ...
  try {
    const body = JSON.parse(Buffer.from(payload, "base64url").toString("utf8"));
    return body.purpose === "client-gallery-session"
      && cleanText(body.clientId, 80)
      && cleanText(body.configuration, 100)
      && Number(body.expiresAt) > Date.now()
      ? body
      : null;
  } catch (error) {
    return null;
...
const requireClient = (req) => {
  const authorization = String(req.headers.authorization || "");
  const token = authorization.startsWith("Bearer ") ? authorization.slice(7) : "";
  const session = verifyClientToken(token);

  if (!session) {
    const error = new Error("Client session is invalid or has expired.");
    error.statusCode = 401;
    throw error;
  }
...
const requireCurrentClient = (session, siteData) => {
  const record = normalisedClientRecords(siteData)
    .find(({ client }) => client.id === session.clientId);

  if (!record || !timingSafeStringEqual(
    session.configuration,
    clientConfigurationFingerprint(record.client, record.storedClient)
  )) {
    const error = new Error("Client session is no longer valid for this gallery.");
    error.statusCode = 401;
    throw error;
  }
...
const publicClientFeedback = (record) => record ? {
  lightroomAssetId: record.lightroomAssetId,
  imageAlt: record.imageAlt,
  imageSrc: `/api/client?action=image&asset=${encodeURIComponent(record.lightroomAssetId)}`,
...
const publicClient = (client, gallery = {}, feedback = []) => ({
  ...
  images: Array.isArray(gallery.images) ? gallery.images.map((image) => ({
    ...
    src: `/api/client?action=image&asset=${encodeURIComponent(image.lightroomAssetId)}`
  })) : [],
  feedback: Array.isArray(feedback) ? feedback.map(publicClientFeedback) : [],
...
const getLightroomGalleryAssets = async (shareUrl) => {
  ...
  let pageUrl = assetsUrl;
  let base = `https://photos.adobe.io/v2/spaces/${share.spaceId}/`;
  while (pageUrl) {
    const page = new URL(pageUrl);
    if (page.origin !== "https://photos.adobe.io" || page.pathname !== new URL(assetsUrl).pathname
      || visitedPages.has(page.href) || visitedPages.size >= 500) {
      throw Object.assign(new Error("The complete Lightroom gallery could not be loaded."), { statusCode: 502 });
    }
    visitedPages.add(page.href);
    const result = await fetchAdobeJson(page.href);
    base = result.base || base;
    collectedAssets.push(...(result.resources || []));
    const next = result.links?.next?.href || result.links?.["/rels/next"]?.href;
    if (next) {
      const nextUrl = new URL(next, next.startsWith("?") ? page.href : base);
      nextUrl.searchParams.set("api_key", lightroomApiKey);
  ...
  for (const [index, resource] of collectedAssets.entries()) {
    ...
    images.push({
      src: renditionUrl,
      // Lightroom Web uses this endpoint for the full-size edited shared photo.
      downloadSrc: `https://dl.lightroom.adobe.com/spaces/${encodeURIComponent(share.spaceId)}/assets/${encodeURIComponent(asset.id)}?gcm=true`,
...
const importLightroomGallery = async (shareUrl) => {
  ...
  const imported = paths.map((src, index) => ({
    src,
    previewSrc: downloadable[index].previewSrc,
...
const withoutPreviewSrc = (item = {}) => {
  const { previewSrc, ...storedItem } = item;
  return storedItem;
};
...
const normaliseSiteData = (siteData) => ({
  ...
  updatedAt: new Date().toISOString(),
  albums: Array.isArray(siteData.albums)
    ? siteData.albums.map((album) => ({
      ...album,
      covers: Array.isArray(album.covers) ? album.covers.map(withoutPreviewSrc) : [],
      images: Array.isArray(album.images) ? album.images.map(withoutPreviewSrc) : []
    }))
    : [],
  clients: normaliseClients(siteData.clients)
});

const loadPublicClientView = async (client) => {
  let gallery = {};
  ...
  try {
    ...
    gallery = {
      error: "The gallery could not be loaded right now. Please try again later."
    };
  }

  const feedbackData = await readClientFeedbackData();
  return publicClient(client, gallery, feedbackForClient(feedbackData, client.id));
};
  ...
  try {
    ...
    if (req.method === "POST" && action === "login") {
      ...
      jsonResponse(res, 200, {
        ok: true,
        token: createClientToken(client, record.storedClient),
        client: await loadPublicClientView(client)
      });
      return;
    }

    if (req.method === "GET" && action === "session") {
      const session = requireClient(req);
      const siteData = await readSiteData();
      const { client } = requireCurrentClient(session, siteData);
    ...
    if (req.method === "GET" && (action === "image" || action === "download")) {
      const session = requireClient(req);
      const { client } = requireCurrentClient(session, await readSiteData());
      if (action === "download" && !client.downloadEnabled) {
        const error = new Error("Downloads are not enabled for this gallery.");
        error.statusCode = 403;
        throw error;
      }
      const gallery = await getLightroomGalleryAssets(client.lightroomUrl);
      const image = gallery.images.find((item) => item.lightroomAssetId === requestUrl.searchParams.get("asset"));
      if (!image) {
        const error = new Error("That image is not part of your gallery.");
        error.statusCode = 404;
        throw error;
      }
      // Resolve upstream URLs on the server; never accept a URL from the browser.
      const chunkSize = 3 * 1024 * 1024;
      const offset = Number(requestUrl.searchParams.get("offset") || 0);
      if (!Number.isSafeInteger(offset) || offset < 0 || offset >= 100 * 1024 * 1024) {
        const error = new Error("Invalid image offset.");
        error.statusCode = 416;
        throw error;
      }
      const upstream = await fetch(action === "download" ? image.downloadSrc : image.src, {
        headers: { range: `bytes=${offset}-${offset + chunkSize - 1}` },
        signal: AbortSignal.timeout(90000)
      });
      const unavailable = () => Object.assign(new Error(action === "download"
        ? "The full-resolution photo is unavailable. Please ask the studio to enable JPG downloads in Lightroom and check that the full-resolution photos are synced."
        : "The image could not be loaded. Please try again."), { statusCode: 502 });
      if (!upstream.ok) throw unavailable();
      const declaredSize = Number(upstream.headers.get("content-length"));
      if (declaredSize > 100 * 1024 * 1024) throw unavailable();
      const parts = [];
      let length = 0;
      if (upstream.body) {
        for await (const chunk of upstream.body) {
          length += chunk.length;
          if (length > 100 * 1024 * 1024) throw unavailable();
      ...
      const contentRange = String(upstream.headers.get("content-range") || "").match(/^bytes (\d+)-(\d+)\/(\d+)$/);
      const total = partial && contentRange ? Number(contentRange[3]) : bytes.length;
      ...
      const type = partial && offset > 0 ? (extension ? { mimeType, extension } : null) : detectImageType(bytes);
      if (!type || total > 100 * 1024 * 1024 || (partial && (!contentRange
        || Number(contentRange[1]) !== offset
        || Number(contentRange[2]) - offset + 1 !== bytes.length
        || Number(contentRange[2]) >= total))) throw unavailable();
      if (offset >= total) {
        const error = new Error("Invalid image offset.");
        error.statusCode = 416;
        throw error;
      ...
      res.setHeader("content-type", type.mimeType);
      res.setHeader("x-content-type-options", "nosniff");
      res.setHeader("x-image-size", String(total));
      res.setHeader("x-image-extension", type.extension);
      if (action === "download") {
        res.setHeader("content-disposition", `attachment; filename="${slugify(image.alt) || "photo"}${type.extension}"`);
      }
      res.end(partial ? bytes : bytes.subarray(offset, offset + chunkSize));
      return;
    }

    if (req.method === "POST" && action === "feedback") {
      const session = requireClient(req);
      const attempt = rateLimitRequest(req, `client-feedback:${session.clientId}`, {
        limit: 90,
        windowMs: 15 * 60 * 1000
      });

      if (!attempt.allowed) {
        const error = new Error("Too many feedback updates. Please try again later.");
        error.statusCode = 429;
        error.retryAfter = attempt.retryAfter;
        throw error;
      }
      ...
      const body = await readRequestJson(req);
      const submission = normaliseFeedbackSubmission(body);
      const siteData = await readSiteData();
      const { client } = requireCurrentClient(session, siteData);
      ...
      const gallery = await getLightroomGalleryAssets(client.lightroomUrl);
      const image = gallery.images.find((item) => (
        String(item.lightroomAssetId || "") === submission.lightroomAssetId
      ));

      if (!image) {
        const error = new Error("That image is no longer part of this client gallery.");
        error.statusCode = 404;
        throw error;
      }

      const result = await saveClientImageFeedback(client, image, submission);
      jsonResponse(res, 200, { ok: true, ...result, feedback: publicClientFeedback(result.feedback) });
      return;
    }
...
const handleAdminRequest = async (req, res) => {
  try {
    ...
    if (req.method === "GET" && action === "site") {
      const [site, feedbackData] = await Promise.all([
        readSiteData(),
        readClientFeedbackData()
      ]);
      jsonResponse(res, 200, { site: mergeClientFeedback(site, feedbackData) });
      return;
    }
    ...
    if (req.method === "POST" && action === "site") {
      ...
      await pruneClientFeedback(site.clients.map((client) => client.id));
      const deployment = await triggerVercelDeploy();
      const feedbackData = await readClientFeedbackData();
      jsonResponse(res, 200, {
        ok: true,
        site: mergeClientFeedback(site, feedbackData),
        deployment
      });
      return;
    }
