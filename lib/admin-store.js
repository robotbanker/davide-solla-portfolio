...
const ensureNewsletterBroadcastConfigured = (config) => {
  if (!config.apiKey) {
    const error = new Error("RESEND_API_KEY is required before sending a newsletter.");
    error.statusCode = 503;
    throw error;
  }
  ...
  if (!config.segmentId) {
    const error = new Error("NEWSLETTER_RESEND_SEGMENT_ID is required before sending a newsletter.");
    error.statusCode = 503;
    throw error;
  }
};
...
const sendNewsletterIssue = async (issueId, confirmation) => {
  ...
  ensureNewsletterBroadcastConfigured(config);
  ...
  const output = newsletterDistPath(issueId);
  await writeDataFile(output, renderEmail(issue), `Build newsletter email ${issueId}`);
  const broadcast = await resendBroadcastRequest("/broadcasts", {
  ...
  return { output, validation, broadcast, deployment };
};
