...
const getLightroomSharedAlbum = async (shareUrl) => {
...
const getLightroomGalleryAssets = async (shareUrl) => {
  const { share, album } = await getLightroomSharedAlbum(shareUrl);
...
const handleClientRequest = async (req, res) => {
  try {
    ...
    if (req.method === "GET" && action === "download") {
      ...
      if (!client.downloadEnabled) {
        throw Object.assign(new Error("Downloads are not enabled for this gallery."), { statusCode: 403 });
      }
      const { share, album } = await getLightroomSharedAlbum(client.lightroomUrl);
      // Lightroom's native gallery export prepares and delivers the full-size ZIP.
      // The website authorizes the request; photo bytes travel directly from Adobe.
      const downloadUrl = `https://dl.lightroom.adobe.com/spaces/${encodeURIComponent(share.spaceId)}/albums/${encodeURIComponent(album.id)}?fullsize=true&gcm=true`;
      jsonResponse(res, 200, { ok: true, downloadUrl });
      return;
    }
    ...
    if (req.method === "GET" && action === "image") {
      ...
      const upstream = await fetch(image.src, {
        headers: { range: `bytes=${offset}-${offset + chunkSize - 1}` },
        signal: AbortSignal.timeout(90000)
      });
      const unavailable = () => Object.assign(new Error("The image could not be loaded. Please try again."), { statusCode: 502 });
