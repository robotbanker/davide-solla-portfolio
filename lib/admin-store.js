...
const ensureNewsletterSenderConfigured = (config) => {
  ...
  if (config.smtpUser && config.smtpPass && config.smtpRecipients.length) {
    return "smtp";
  }

  if (!config.smtpUser || !config.smtpPass) {
    const error = new Error("Either RESEND_API_KEY with NEWSLETTER_RESEND_SEGMENT_ID, or SMTP_USER and SMTP_PASS, is required before sending a newsletter.");
    error.statusCode = 503;
    throw error;
  }

  const error = new Error("NEWSLETTER_RECIPIENTS, NEWSLETTER_TO_EMAIL, or CONTACT_TO_EMAIL is required before sending with SMTP.");
  error.statusCode = 503;
  throw error;
};
...
const sendNewsletterWithSmtp = async (issue, html, config) => {
  ...
  const hasMultipleRecipients = config.smtpRecipients.length > 1;

  await transporter.sendMail({
    from: config.fromEmail,
    to: hasMultipleRecipients ? config.fromEmail : config.smtpRecipients[0],
    bcc: hasMultipleRecipients ? config.smtpRecipients : undefined,
  ...
  return {
    provider: "smtp",
    recipientCount: config.smtpRecipients.length
  };
};

const sendNewsletterIssue = async (issueId, confirmation) => {
  ...
  const provider = ensureNewsletterSenderConfigured(config);

  const issue = await readNewsletterIssue(issueId);
  const validation = validateIssue(issue, await readNewsletterManifest(issueId), { strict: true });
  ...
  const delivery = provider === "resend"
    ? await resendBroadcastRequest("/broadcasts", {
    ...
    : await sendNewsletterWithSmtp(issue, html, config);
  const deployment = await triggerVercelDeploy();
...
const handleAdminRequest = async (req, res) => {
  try {
    ...
    if (req.method === "POST" && action === "newsletterSend") {
      const issueId = requestUrl.searchParams.get("issueId") || "";
      const body = await readRequestJson(req);
      const result = await sendNewsletterIssue(issueId, body.confirmation || "");
      jsonResponse(res, 200, { ok: true, ...result });
      return;
    }
