...
const publicClient = (client, gallery = {}, feedback = []) => ({
  ...
  email: client.email || "",
  downloadEnabled: client.downloadEnabled === true,
  lightroomUrl: client.downloadEnabled === true
    ? gallery.lightroomUrl || client.lightroomUrl || ""
    : "",
...
const loadPublicClientView = async (client) => {
  ...
  try {
    ...
    gallery = {
      error: client.downloadEnabled
        ? "The embedded preview could not be loaded, but the Lightroom download link is available."
        : "The embedded preview could not be loaded right now."
    };
  }
