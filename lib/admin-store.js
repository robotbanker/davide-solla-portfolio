...
let newsletterWriteLock = Promise.resolve();

const withNewsletterWriteLock = async (operation) => {
  const previous = newsletterWriteLock;
  let release;
  const current = new Promise((resolve) => { release = resolve; });
  newsletterWriteLock = current;
  ...
  try {
    ...
    release();
    if (newsletterWriteLock === current) {
      newsletterWriteLock = Promise.resolve();
    }
  }
};
...
const writeNewsletterIssue = (issueId, issue, manifest, expectedRevision) => withNewsletterWriteLock(
