...
const renderIssueImage = (definition, { eager = false } = {}) => {
  ...
  return `<figure class="field-image">
            <img src="${escapeHtml(url)}" alt="${escapeHtml(alt)}" loading="${eager ? "eager" : "lazy"}" decoding="async"${eager ? " fetchpriority=\"high\"" : ""}>
  ...
  return `<header class="issue-head">
        ...
        <section class="field-section">
          ...
          <article class="field-feature">
            ${renderIssueImage(artDefinition, { eager: true })}
...
<html lang="en-GB" data-analytics="enabled">
  <head>
    <script src="/privacy-consent.js?v=2026-08-19" defer></script>
  ...
  <body>
    <a class="skip-link" href="#current-issue">Skip to current issue</a>
    <header class="site-header is-scrolled" data-header>
      ...
      <div class="header-actions">
        ...
        <button class="menu-toggle" type="button" aria-label="Open navigation" aria-controls="site-navigation" aria-expanded="false" data-menu-toggle><span></span><span></span></button>
      </div>
      <nav class="site-nav" id="site-navigation" data-nav aria-label="Main navigation">
    ...
    <main class="field-notes-page" id="top">
      ...
      <section class="field-notes-shell" aria-label="Field Notes issue archive">
        ...
        <article class="field-notes-issue" id="current-issue" data-field-notes-issue data-field-notes-prerendered tabindex="-1">${renderIssueContent(issue, manifest)}</article>
      </section>
    </main>
