...
const slotApproval = (issue, manifest, definition) => imageApproval(
  issue,
  manifest,
  definition,
  ["public-web"]
...
const socialImageForIssue = (issue, manifest) => {
  for (const definition of renderedImageSlots(issue)) {
    const approval = slotApproval(issue, manifest, definition);
    if (approval.approved) {
      return {
        url: approval.assetUrl,
...
const renderApprovedImage = (issue, manifest, definition, { eager = false } = {}) => {
  const approval = slotApproval(issue, manifest, definition);
  if (!approval.approved) return "";
  const alt = cleanText(definition.image?.alt, 300);
  return `<figure class="field-image">
            <img src="${escapeHtml(approval.assetUrl)}" alt="${escapeHtml(alt)}" loading="${eager ? "eager" : "lazy"}" decoding="async"${eager ? " fetchpriority=\"high\"" : ""}>
...
const renderIssueContent = (issue, manifest) => {
  ...
  const fieldSection = fieldNote ? `<section class="field-section">
          ...
          ${renderApprovedImage(issue, manifest, {
            slot: "onTheField",
            image: fieldImage,
  ...
  return `<header class="issue-head">
        ...
        <section class="field-section">
          ...
          <article class="field-feature">
            ${renderApprovedImage(issue, manifest, artDefinition, { eager: true })}
        ...
        <section class="field-section">
          ...
          ${(fashion.stories || []).map((story, index) => {
            ...
            ${renderApprovedImage(issue, manifest, definition)}
...
<html lang="en-GB" data-analytics="enabled">
  <head>
    ...
    <link rel="stylesheet" href="/field-notes.css?v=5">
