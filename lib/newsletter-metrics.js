...
const privateSource = (value) => {
  ...
  if (!source || Buffer.byteLength(source, "utf8") > 2048) {
    fail("fact_invalid");
  }
  return source;
};
...
const createNewsletterCampaignKey = (rawResendBroadcastId, secret = process.env.NEWSLETTER_METRICS_WEBHOOK_SECRET) => (
...
const createNewsletterLifecycleEventId = (
  type,
  eventSource,
  secret = process.env.NEWSLETTER_METRICS_WEBHOOK_SECRET
) => {
  if (!newsletterLifecycleTypes.has(type)) {
    fail("fact_invalid");
  }
...
const buildNewsletterLifecycleFact = (observation, secret = process.env.NEWSLETTER_METRICS_WEBHOOK_SECRET) => {
  ...
  if (observation.issueId !== undefined) {
  ...
  if (observation.campaignSource !== undefined) {
...
const validateNewsletterLifecycleFact = (fact) => {
  ...
  if (Object.keys(fact).some((key) => !allowedKeys.has(key))) {
    fail("fact_invalid");
  }
  ...
  if (fact.schema_version !== 1
    ...
    || (fact.issue_id !== undefined && !issueIdPattern.test(String(fact.issue_id)))
    || (fact.campaign_key !== undefined && !campaignKeyPattern.test(String(fact.campaign_key)))) {
    fail("fact_invalid");
  }
...
const newsletterMetricsConfig = () => {
  ...
  return {
    ...
    secret: String(process.env.NEWSLETTER_METRICS_WEBHOOK_SECRET || ""),
...
const resolveMetricsEndpoint = (config) => {
  if (!config.endpoint || !hasStrongSecret(config.secret)) {
    fail("configuration_invalid");
  }
...
const postNewsletterLifecycleFact = async (fact, options = {}) => {
  ...
  const signature = hmacHex(config.secret, `${timestamp}.${body}`);
  ...
  try {
    ...
    if (error instanceof NewsletterMetricsError) {
      throw error;
    }
...
const observeNewsletterLifecycle = async (observation) => {
  try {
    ...
    const fact = buildNewsletterLifecycleFact(observation, config.secret);
