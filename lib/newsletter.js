...
const preferenceEventSource = (token, transition) => {
  const tokenDigest = crypto
    .createHash("sha256")
    .update(String(token || ""), "utf8")
    .digest("hex");
  return `${transition}:preference-token:${tokenDigest}`;
};
...
const confirmationHtml = (url, config) => `
  <div style="background:#09090b;color:#f0f0ee;font-family:Avenir Next,Helvetica,Arial,sans-serif;margin:0;padding:32px;">
    <div style="margin:0 auto;max-width:560px;">
      <p style="color:#d6d6d8;font-size:12px;font-weight:700;margin:0 0 16px;text-transform:uppercase;">${escapeHtml(config.listName)}</p>
      ...
      <p style="color:#cecece;font-size:16px;line-height:1.65;margin:0 0 24px;">Please confirm that you want to receive ${escapeHtml(config.listName)} emails from ${escapeHtml(config.brandName)}.</p>
      <p style="margin:0 0 28px;"><a href="${escapeHtml(url)}" style="border:1px solid rgba(240,240,238,0.48);color:#f0f0ee;display:inline-block;font-size:12px;font-weight:700;padding:14px 18px;text-decoration:none;text-transform:uppercase;">Confirm subscription</a></p>
      <p style="color:#a8a8aa;font-size:13px;line-height:1.6;margin:0;">If this was not you, you can ignore this email.</p>
    </div>
  </div>
`;
...
const preferencesEmailHtml = (url, config) => `
  <div style="background:#09090b;color:#f0f0ee;font-family:Avenir Next,Helvetica,Arial,sans-serif;margin:0;padding:32px;">
    <div style="margin:0 auto;max-width:560px;">
      ...
      <p style="color:#cecece;font-size:16px;line-height:1.65;margin:0 0 24px;">Use this private link to review or change your ${escapeHtml(config.listName)} subscription. It expires in 30 minutes.</p>
      <p style="margin:0 0 28px;"><a href="${escapeHtml(url)}" style="border:1px solid rgba(240,240,238,0.48);color:#f0f0ee;display:inline-block;font-size:12px;font-weight:700;padding:14px 18px;text-decoration:none;text-transform:uppercase;">Open preferences</a></p>
      <p style="color:#a8a8aa;font-size:13px;line-height:1.6;margin:0;">If you did not request this link, you can ignore this email.</p>
    </div>
  </div>
`;
...
const handleUpdatePreferences = async (req, res) => {
  ...
  if (body.unsubscribeAll === true) {
    ...
    await observeNewsletterLifecycle({
      ...
      occurredAt,
      eventSource: preferenceEventSource(body.token, "global-opt-out")
    });
    jsonResponse(res, 200, {
      ok: true,
  ...
  if (body.fieldNotes === false) {
    await updateContactTopic(contactId, config, "opt_out");
    const occurredAt = new Date().toISOString();
    await observeNewsletterLifecycle({
      type: "subscription.topic_opted_out",
      occurredAt,
      eventSource: preferenceEventSource(body.token, "topic-opt-out")
    });
    jsonResponse(res, 200, {
      ok: true,
      preferences: {
        globallySubscribed: null,
        fieldNotes: false,
        topicConfigured: true
      },
      message: "You will no longer receive Field Notes."
    });
    return;
  }
  ...
  if (topicPreference.fieldNotes === true) {
    jsonResponse(res, 200, {
      ok: true,
      preferences: {
        globallySubscribed: contact.unsubscribed !== true,
      ...
      message: "Field Notes remains selected."
    });
    return;
  }

  await updateContactTopic(contactId, config, "opt_in");
  jsonResponse(res, 200, {
    ok: true,
    preferences: {
      globallySubscribed: contact.unsubscribed !== true,
